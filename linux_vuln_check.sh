...
latest_check_u02() {
    ...
    local pwquality_files=()
    [ -f "$pwquality_conf" ] && pwquality_files+=("$pwquality_conf")
    ...
    get_pwquality_value() {
        ...
        awk -F= -v key="$key" '
            /^[[:space:]]*#/ || NF < 2 { next }
            {
                ...
                if (tolower(name) == key) result=value
            }
            END { print result }
        ' "${pwquality_files[@]}" 2>/dev/null
    }
    ...
    if [[ "$minlen" =~ ^[0-9]+$ ]] && [ "$minlen" -ge 9 ]; then
        ...
        fail_reasons="${fail_reasons}minlen < 9 (${minlen:-not set}); "
    fi
    if [[ "$dcredit" =~ ^-?[0-9]+$ ]] && [ "$dcredit" -lt 0 ]; then pass_details="${pass_details}digit required, "; else fail_reasons="${fail_reasons}digit requirement missing (dcredit=${dcredit:-not set}); "; fi
    if [[ "$ucredit" =~ ^-?[0-9]+$ ]] && [ "$ucredit" -lt 0 ]; then pass_details="${pass_details}uppercase required, "; else fail_reasons="${fail_reasons}uppercase requirement missing (ucredit=${ucredit:-not set}); "; fi
    if [[ "$lcredit" =~ ^-?[0-9]+$ ]] && [ "$lcredit" -lt 0 ]; then pass_details="${pass_details}lowercase required, "; else fail_reasons="${fail_reasons}lowercase requirement missing (lcredit=${lcredit:-not set}); "; fi
    if [[ "$ocredit" =~ ^-?[0-9]+$ ]] && [ "$ocredit" -lt 0 ]; then pass_details="${pass_details}special char required, "; else fail_reasons="${fail_reasons}special char requirement missing (ocredit=${ocredit:-not set}); "; fi

    if [ -f "$login_defs" ]; then
        pass_max_days=$(awk '$1=="PASS_MAX_DAYS" {print $2}' "$login_defs" | tail -1)
        pass_min_days=$(awk '$1=="PASS_MIN_DAYS" {print $2}' "$login_defs" | tail -1)
        if [[ "$pass_max_days" =~ ^[0-9]+$ ]] && [ "$pass_max_days" -le 90 ]; then pass_details="${pass_details}PASS_MAX_DAYS=${pass_max_days}, "; else fail_reasons="${fail_reasons}PASS_MAX_DAYS > 90 or not set (${pass_max_days:-not set}); "; fi
        if [[ "$pass_min_days" =~ ^[0-9]+$ ]] && [ "$pass_min_days" -ge 1 ]; then pass_details="${pass_details}PASS_MIN_DAYS=${pass_min_days}"; else fail_reasons="${fail_reasons}PASS_MIN_DAYS < 1 or not set (${pass_min_days:-not set})"; fi
    else
        fail_reasons="${fail_reasons}${login_defs} not found"
    fi
...
latest_check_u03() {
    ...
    local deny unlock_time even_deny_root pam_faillock=false authselect_faillock=false
    deny=$(grep -Ei '^[[:space:]]*deny[[:space:]]*=' "$faillock_conf" | grep -v '^[[:space:]]*#' | tail -1 | awk -F= '{print $2}' | tr -d ' ')
    unlock_time=$(grep -Ei '^[[:space:]]*unlock_time[[:space:]]*=' "$faillock_conf" | grep -v '^[[:space:]]*#' | tail -1 | awk -F= '{print $2}' | tr -d ' ')
    even_deny_root=$(grep -Ei '^[[:space:]]*even_deny_root' "$faillock_conf" | grep -v '^[[:space:]]*#')

    if [[ "$deny" =~ ^[0-9]+$ ]] && [ "$deny" -gt 0 ] && [ "$deny" -le 5 ]; then pass_details="${pass_details}deny=${deny}, "; else fail_reasons="${fail_reasons}deny should be 1-5 (${deny:-not set}); "; fi
    if [[ "$unlock_time" =~ ^[0-9]+$ ]] && [ "$unlock_time" -gt 0 ]; then pass_details="${pass_details}unlock_time=${unlock_time}, "; else fail_reasons="${fail_reasons}unlock_time not set or invalid (${unlock_time:-not set}); "; fi
    [ -n "$even_deny_root" ] && fail_reasons="${fail_reasons}even_deny_root is set; " || pass_details="${pass_details}root excluded, "

    if grep -Rqs 'pam_faillock\.so' /etc/pam.d/system-auth /etc/pam.d/password-auth 2>/dev/null; then pam_faillock=true; fi
    if command -v authselect >/dev/null 2>&1 && authselect current 2>/dev/null | grep -q 'with-faillock'; then authselect_faillock=true; fi
    if [ "$pam_faillock" = true ] || [ "$authselect_faillock" = true ]; then
        pass_details="${pass_details}faillock PAM/authselect enabled"
    else
        fail_reasons="${fail_reasons}pam_faillock/authselect with-faillock not verified"
    fi

    if [ -z "$fail_reasons" ]; then log_result "$check_id" "$check_name" "PASS" "[Risk: ${risk_level}] ${pass_details}"; else log_result "$check_id" "$check_name" "FAIL" "[Risk: ${risk_level}] ${fail_reasons}"; fi
}
...
latest_check_u12() {
    ...
    local risk_level="MEDIUM"
    local files=(/etc/profile /etc/bashrc /etc/bash.bashrc)
    local found=false good=false details="" issues=""

    while IFS= read -r file; do files+=("$file"); done < <(find /etc/profile.d -maxdepth 1 -type f -name '*.sh' 2>/dev/null | sort)

    for file in "${files[@]}"; do
        [ -f "$file" ] || continue
        if grep -Eq '^[[:space:]]*TMOUT=' "$file"; then
            found=true
            local value
            value=$(grep -E '^[[:space:]]*TMOUT=' "$file" | tail -1 | sed 's/.*TMOUT=//' | sed 's/[;[:space:]].*//')
            if [[ "$value" =~ ^[0-9]+$ ]] && [ "$value" -gt 0 ] && [ "$value" -le 900 ]; then
                if grep -Eq '^[[:space:]]*(readonly[[:space:]]+TMOUT|declare[[:space:]]+-r[[:space:]]+TMOUT)' "$file" && grep -Eq '^[[:space:]]*export[[:space:]]+TMOUT' "$file"; then
                    good=true; details="${file}: TMOUT=${value}, readonly/export set"; break
                else
                    issues="${issues}${file}: TMOUT=${value} but readonly/export missing; "
                fi
            else
                issues="${issues}${file}: TMOUT invalid or >900 (${value:-not set}); "
            fi
        fi
    done

    if [ "$good" = true ]; then log_result "$check_id" "$check_name" "PASS" "[Risk: ${risk_level}] ${details}"; elif [ "$found" = true ]; then log_result "$check_id" "$check_name" "FAIL" "[Risk: ${risk_level}] ${issues}"; else log_result "$check_id" "$check_name" "FAIL" "[Risk: ${risk_level}] TMOUT is not configured in system shell profiles"; fi
}
...
latest_check_u37() {
    ...
    check_cron_file() {
        ...
        [ "$owner" != "root:root" ] && issues="${issues}${file} owner=${owner}; "
        if [ "$mode" -gt "$max_mode" ] 2>/dev/null; then issues="${issues}${file} mode=${mode} > ${max_mode}; "; fi
    }

    check_cron_file /etc/crontab 640
    check_cron_file /etc/cron.allow 640
    check_cron_file /etc/cron.deny 640
    for file in /etc/cron.d/* /var/spool/cron/*; do [ -e "$file" ] && check_cron_file "$file" 640; done
    if [ -e /usr/bin/crontab ]; then
        checked=$((checked + 1))
        local owner mode
        owner=$(stat -c '%U:%G' /usr/bin/crontab 2>/dev/null)
        mode=$(stat -c '%a' /usr/bin/crontab 2>/dev/null)
        [ "$owner" != "root:root" ] && issues="${issues}/usr/bin/crontab owner=${owner}; "
        if [ $((8#$mode & 2)) -ne 0 ]; then issues="${issues}/usr/bin/crontab is world-writable (mode=${mode}); "; fi
    fi

    if [ "$checked" -eq 0 ]; then log_result "$check_id" "$check_name" "N/A" "[Risk: ${risk_level}] cron configuration files were not found"; elif [ -z "$issues" ]; then log_result "$check_id" "$check_name" "PASS" "[Risk: ${risk_level}] ${checked} cron files/binaries have acceptable owner and permissions"; else log_result "$check_id" "$check_name" "FAIL" "[Risk: ${risk_level}] ${issues}"; fi
}
...
latest_check_u58() {
    ...
    if { command -v systemctl >/dev/null 2>&1 && systemctl is-active --quiet snmpd 2>/dev/null; } || pgrep -x snmpd >/dev/null 2>&1; then
        log_result "$check_id" "$check_name" "FAIL" "[Risk: ${risk_level}] SNMP service is running; disable it if not explicitly required"
    else
        log_result "$check_id" "$check_name" "PASS" "[Risk: ${risk_level}] SNMP service is not active"
    fi
}
...
latest_check_u60() {
    local check_id="U-60"
    local check_name="SNMP Community String 복잡성 설정"
    local risk_level="MEDIUM"
    ...
    [ -f "$conf" ] || { log_result "$check_id" "$check_name" "FAIL" "[Risk: ${risk_level}] SNMP is running but ${conf} was not found"; return; }
    local communities weak="" count=0
    communities=$(awk '
        /^[[:space:]]*#/ { next }
        { key=tolower($1) }
        key=="rocommunity" || key=="rwcommunity" { print $2 }
        key=="com2sec" { print $4 }
    ' "$conf" 2>/dev/null)
    while IFS= read -r community; do
        [ -n "$community" ] || continue
        count=$((count + 1))
        if [[ "$community" =~ ^(public|private)$ ]] || [ ${#community} -lt 10 ]; then weak="${weak}${community} "; fi
    done <<< "$communities"
    if [ "$count" -eq 0 ]; then log_result "$check_id" "$check_name" "PASS" "[Risk: ${risk_level}] SNMP v1/v2 community strings are not configured"; elif [ -z "$weak" ]; then log_result "$check_id" "$check_name" "PASS" "[Risk: ${risk_level}] ${count} SNMP community strings are non-default and length >= 10"; else log_result "$check_id" "$check_name" "FAIL" "[Risk: ${risk_level}] Weak/default SNMP community strings found: ${weak}"; fi
}

latest_check_u64() {
    local check_id="U-64"
    local check_name="주기적 보안 패치 및 벤더 권고사항 적용"
    local risk_level="HIGH"
    local pm="$CAP_PACKAGE_MANAGER" security_updates="" normal_updates=""
    [ "$pm" = "unknown" ] && { log_result "$check_id" "$check_name" "N/A" "[Risk: ${risk_level}] Package manager capability is unknown"; return; }
    if [ "$pm" = "dnf" ]; then
        security_updates=$(dnf -q updateinfo list security updates 2>/dev/null | grep -Ec '^[A-Z]|^RHSA|^RLSA' || true)
        normal_updates=$(dnf -q check-update 2>/dev/null | grep -Ec '^[[:alnum:]_.+-]+[[:space:]]' || true)
    elif [ "$pm" = "yum" ]; then
        security_updates=$(yum -q updateinfo list security updates 2>/dev/null | grep -Ec '^[A-Z]|^RHSA|^RLSA' || true)
        normal_updates=$(yum -q check-update 2>/dev/null | grep -Ec '^[[:alnum:]_.+-]+[[:space:]]' || true)
    else
        log_result "$check_id" "$check_name" "N/A" "[Risk: ${risk_level}] Only rpm is available; security update status requires manual verification"
        return
    fi
    if [ "${security_updates:-0}" -gt 0 ] 2>/dev/null; then
        log_result "$check_id" "$check_name" "FAIL" "[Risk: ${risk_level}] ${security_updates} security updates are available via ${pm}"
    elif [ "${normal_updates:-0}" -gt 0 ] 2>/dev/null; then
        log_result "$check_id" "$check_name" "N/A" "[Risk: ${risk_level}] No security updates reported, but ${normal_updates} general updates are available; review vendor advisories"
    else
        log_result "$check_id" "$check_name" "PASS" "[Risk: ${risk_level}] No pending security updates reported by ${pm}"
    fi
}
...
latest_check_u63() {
    ...
    local risk_level="MEDIUM"
    local issues="" files=(/etc/sudoers) policy_count=0
    ...
    if [ -d /etc/sudoers.d ]; then
        ...
        dir_owner=$(stat -c '%U:%G' /etc/sudoers.d 2>/dev/null)
        dir_mode=$(stat -c '%a' /etc/sudoers.d 2>/dev/null)
        [ "$dir_owner" != "root:root" ] && issues="${issues}/etc/sudoers.d owner=${dir_owner}; "
        if [[ "$dir_mode" =~ ^[0-7]+$ ]] && [ $((8#$dir_mode & 022)) -ne 0 ]; then
            issues="${issues}/etc/sudoers.d is group/other-writable (mode=${dir_mode}); "
        fi
    ...
    for file in "${files[@]}"; do
        ...
        [ "$owner" != "root:root" ] && issues="${issues}${file} owner=${owner}; "
        if [[ "$mode" =~ ^[0-7]+$ ]]; then
            if [ $((8#$mode & 022)) -ne 0 ] || [ "$mode" -gt 440 ] 2>/dev/null; then
                issues="${issues}${file} mode=${mode}; "
            fi
        else
            issues="${issues}${file} mode could not be verified; "
        fi

        if awk '
            /^[[:space:]]*($|#)/ { next }
            /^[[:space:]]*Defaults([[:space:]]|:|$)/ { next }
            /^[[:space:]]*(#include|#includedir|@include|@includedir)/ { next }
            /NOPASSWD[[:space:]]*:/ { found=1 }
            END { exit found ? 0 : 1 }
        ' "$file" 2>/dev/null; then
            issues="${issues}${file} contains NOPASSWD sudo rule; "
        fi

        policy_count=$((policy_count + $(awk '
            /^[[:space:]]*($|#)/ { next }
            /^[[:space:]]*Defaults([[:space:]]|:|$)/ { next }
            /^[[:space:]]*(#include|#includedir|@include|@includedir)/ { next }
    ...
    [ "$policy_count" -eq 0 ] && issues="${issues}no active sudo policy entries found; "

    if [ -z "$issues" ]; then
        log_result "$check_id" "$check_name" "PASS" "[Risk: ${risk_level}] sudoers files are protected, active policies exist, and NOPASSWD rules were not found"
    else
